...
function Level:init()
    ...
    -- Has any collision occured with the Player
...
function Level:update(dt)
	...
	if love.keyboard.wasPressed('space') and not self.isCollision then
	    self.isCollision = true
	    local alien = Alien(self.world, 'circle', xPos, yPos-ALIEN_SIZE, 'Player')
	    local angle = 0
	    local xVelCur, yVelCur = 0, 0
	    angle = math.pi/12
	    xVelCur = xVel*math.cos(angle) - yVel*math.sin(angle)
	    yVelCur = yVel*math.cos(angle) - xVel*math.sin(angle)
	    alien.body:setLinearVelocity(xVelCur, yVelCur)
	    table.insert(self.launchMarker.aliens, alien)
	    alien = Alien(self.world, 'circle', xPos, yPos+ALIEN_SIZE, 'Player')
	    angle = -math.pi/6
	    xVelCur = xVel*math.cos(angle) - yVel*math.sin(angle)
	    yVelCur = yVel*math.cos(angle) - xVel*math.sin(angle)
	    alien.body:setLinearVelocity(xVelCur, yVelCur)
	...
	for _, alien in pairs(self.launchMarker.aliens) do
	    local xPosCur, yPosCur = alien.body:getPosition()
	    local xVelCur, yVelCur = alien.body:getLinearVelocity()
	    xPos = math.max(xPos, xPosCur)
	    xVel = math.max(math.abs(xVel), math.abs(xVelCur))
	    yVel = math.max(math.abs(yVel), math.abs(yVelCur))
	end

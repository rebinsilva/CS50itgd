...
function PlayState:enter(params)
    ...
    -- to keep check the growth of paddle
    self.score_mark = 0
end
...
function PlayState:update(dt)
    ...
    local remove_powerup = {}
    for i, powerup in ipairs(self.powerups) do
	if powerup:collides(self.paddle) then
	    remove_powerup[#remove_powerup+1] = i
    ...
    for _, i in ipairs(remove_powerup) do
	...
	for k, brick in pairs(self.bricks) do
		...
		if math.random(2) < 2 then
		    table.insert(self.powerups, PowerUp(brick.x, brick.y, self.isAnyLocked))
		end
    ...
    if self.score > self.score_mark + 10000 then
	self.score_mark = self.score
	self.paddle:grow()
    end

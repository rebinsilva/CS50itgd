...
function PlayState:init()
    local difficulty = 1
    local score = 0
    self:enter({difficulty = difficulty, score = score})
end
...
function PlayState:enter(params)
    ...
    self.level = LevelMaker.generate(self.difficulty)
    ...
    local nonChasmCol = self.tileMap.width
    for i = 1, self.tileMap.width do
	local isChasm = true
	for j = 1, self.tileMap.height do
	    if self.tileMap.tiles[j][i].id == TILE_ID_GROUND then
		isChasm = false
	    end
	end
	if not isChasm then
